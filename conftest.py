...
def pytest_addoption(parser):
    parser.addoption('--backend', default='CPU',
                     choices=['INTERPRETER', 'CPU', 'GPU', 'NNP', 'PlaidML', 'INTELGPU', 'IE'],
...
def pytest_collection_modifyitems(config, items):
    ...
    ie_skip = pytest.mark.skip(reason='Skipping test on the IE backend.')
    ...
    for item in items:
        ...
        if backend_name == 'IE' and 'skip_on_ie' in item.keywords:
